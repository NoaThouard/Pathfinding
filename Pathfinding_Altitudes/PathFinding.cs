    ...
    class PathFinding
    {
        ...
        public List<Node> CalculatePath()
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                {
                    return CreatePath(startingNode, goalNode);
                }
                ...
                foreach (Node neighbour in GetNeighbours(currentNode))
                {
                    ...
                    //calculate costs
                    int movementCost = currentNode.gCost + ManhattanDistance(currentNode, neighbour) + CalculateSlope(currentNode, neighbour);
        ...
        //Gets all surrounding neighbours, diagonals included
        ...
        //Calculates slope using altitude
        public int CalculateSlope(Node node, Node _node)
        {
            if (node == _node)
            {
                //return 0 if there is no slope i.e no change in altitude
                return 0;
            }
            int nMax = Math.Max(node.altitude, _node.altitude);
            int nMin = Math.Min(node.altitude, _node.altitude);
            int rise = nMax - nMin;
            //distance aka default movementCost i.e 10
            //int run = 10;
            int sum = rise;/// run);
            return sum;
        }
        ...
        List<Node> CreatePath(Node start, Node end)
        {
            path = new List<Node>();
